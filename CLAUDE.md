...
- **ARIA orchestrator** : point d'entrée AI unique. Ne jamais écrire vers l'ancien `#chatMessages v1`. Tous les appels AI passent par la **Supabase Edge Function `ai-proxy`** (source dans `supabase/functions/ai-proxy/index.ts`), jamais d'appel direct provider depuis le client. La function walk la cascade `AI_ROUTES` (12 tasks × 4 tiers) côté serveur ; la même table est dupliquée côté client en fallback si la function elle-même est down (line ~21761). Pour modifier le routing : éditer le source + redeploy (`mcp deploy_edge_function` ou Supabase CLI). Intent classification heuristique route vers : `reason`, `write_fr`, `extract`, `fast_classify`, `chat_fr`. Override manuel : préfixe `/deep`. *La mention historique d'un Cloudflare Worker `stafflo-proxy.jeremiegougenheim.workers.dev` est obsolète — tout est sur Supabase depuis le déploiement v21 de `ai-proxy`.*
